...
namespace Pixel.Rhino.Geometry
{
    ...
    public class Polyline : Pixel.Rhino.Collections.Point3dList, ICloneable
    {
        ...
        /// <summary>
        /// Gets the parameter on the polyline at the given length. 
        /// The integer part of the parameter indicates the index of the segment.
        /// </summary>
        /// <param name="length">Lenth to evaluate.</param>
        /// <returns>The parameter on the polyline at length from start.</returns>
        /// <since>5.0</since>
        [ConstOperation]
        public double ParameterAtLength(double length)
        {
            if (length > Length) return (double)SegmentCount;
            if (length <= 0.0) return 0.0;
            double additive_length = 0.0;
            for (int i = 0; i < SegmentCount; i++)
            {
                Line seg = SegmentAt(i);
                additive_length += seg.Length;
                if (additive_length > length)
                {
                    double localLength = length - (additive_length - seg.Length);
                    return i + localLength / seg.Length;
                }
            }
            return 0.0;
        }
        ...
        /// </summary>
        /// <param name="testPoint">Point to approximate.</param>
        /// <param name="t">Parameter af found point on line.</param>
        /// <returns>The point on the polyline closest to testPoint.</returns>
        /// <since>5.0</since>
        [ConstOperation]
        public Point3d ClosestPoint(Point3d testPoint, out double t)
        {
            t = 0.0;
            if (Count == 0) { return Point3d.Unset; }
            t = ClosestParameter(testPoint);
            return PointAt(t);
        }
        ...
        /// <since>5.0</since>
        [ConstOperation]
        public Point3d ClosestPoint(Point3d testPoint, int segmentFrom, int segmentTo)
        {
            if (Count == 0) { return Point3d.Unset; }
            double t = ClosestParameter(testPoint, segmentFrom, segmentTo);
            return PointAt(t);
        }
        ...
        /// <since>5.0</since>
        [ConstOperation]
        public Point3d ClosestPoint(Point3d testPoint, out double t, int segmentFrom, int segmentTo)
        {
            t = 0.0;
            if (Count == 0) { return Point3d.Unset; }
            t = ClosestParameter(testPoint, segmentFrom, segmentTo);
            return PointAt(t);
        }
        ...
        /// <since>5.0</since>
        [ConstOperation]
        public double ClosestParameter(Point3d testPoint, int segmentFrom, int segmentTo)
        {
            int count = Count;
            if (count < 2) { return 0.0; }
            if (segmentTo < segmentFrom) { return 0.0; }

            int s_min = 0;
            double t_min = 0.0;
            double d_min = double.MaxValue;

            for (int i = Math.Max(0,segmentFrom); i < Math.Min(segmentTo, count-1); i++)
            {
                Line seg = new Line(this[i], this[i + 1]);
                double d;
                double t;
        ...
        /// <summary>
        /// Comput proximity between this and othe Polyline
        /// </summary>
        ...
        [ConstOperation]
        public Tuple<Point3d, Point3d> ProximityOLD(Polyline other, out Tuple<double, double, double> parameters)

        {
            List<Tuple<double, double, double>> result = new List<Tuple<double, double, double>>();
            for (int i = 0; i < this.Count; i++)
            {
                double t = other.ClosestParameter(this[i]);
                double sqDist = other.PointAt(t).DistanceToSquared(this[i]);
                result.Add(Tuple.Create(sqDist, (double)i, t));
            }
            for (int i = 0; i < other.Count; i++)
            {
                double t = this.ClosestParameter(other[i]);
                double sqDist = this.PointAt(t).DistanceToSquared(other[i]);
                result.Add(Tuple.Create(sqDist, t, (double)i));
            }
        ...
        /// <summary>
        /// Compute area of the polyline
        /// </summary>
        /// <returns>Polyline area; only if closed, else area will be 0.0.</returns>
        public double Area()
        {
            if (!this.IsClosed) return 0.0;

            double area = Math.Abs(this.Take(this.Count - 1)
